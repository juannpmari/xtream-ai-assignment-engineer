...
class Trainer(ABC):
    ...
    def load_data(self):
        ...
        try:
            ...
            logging.info(f"Loaded X:{len(self.X_train)}, y:{len(self.y_train)} for train set")
            logging.info(f"Loaded X:{len(self.X_test)}, y:{len(self.y_test)} for test set")
...
class RegTreeTrainer(Trainer):
    def __init__(self,current_time,config):
        super().__init__(current_time,config)
